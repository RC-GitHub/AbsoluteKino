...
router.post(
  ...
  Auth.validatePrivileges("movies", 3), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const {
      title,
      viewingFormat,
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: Messages.MOVIE_ERR_EMPTY_ARGS, movies: [] });
    }
    ...
    if (
      trimmedTitle.length < Constants.MOVIE_TITLE_MIN_LEN ||
      trimmedTitle.length > Constants.MOVIE_TITLE_MAX_LEN
    ) {
      return res
        .status(400)
    ...
    if (
      duration < Constants.MOVIE_DUR_MIN ||
      duration > Constants.MOVIE_DUR_MAX
    ) {
      return res
        .status(400)
        .json({ message: Messages.MOVIE_ERR_DURATION, movies: [] });
    }

    const trimmedDesc = description.trim();
    if (
      trimmedDesc.length < Constants.MOVIE_DESC_MIN_LEN ||
      trimmedDesc.length > Constants.MOVIE_DESC_MAX_LEN
    ) {
      return res
        .status(400)
        .json({ message: Messages.MOVIE_ERR_DESC, movies: [] });
    }

    if (!isValidURL(posterUrl)) {
      return res
        .status(400)
        .json({ message: Messages.MOVIE_ERR_POSTER_URL, movies: [] });
    }

    if (!isValidURL(trailerUrl)) {
      return res
        .status(400)
        .json({ message: Messages.MOVIE_ERR_TRAILER_URL, movies: [] });
    }

    const trimmedLang = language.trim();
    if (
      trimmedLang.length < Constants.MOVIE_LANG_MIN_LEN ||
      trimmedLang.length > Constants.MOVIE_LANG_MAX_LEN
    ) {
      return res
        .status(400)
        .json({ message: Messages.MOVIE_ERR_LANG_LEN, movies: [] });
    }
    ...
    if (
      !(premiereDate as unknown as string).includes(
    ...
    if (
      trimmedGenre.length < Constants.MOVIE_GENRE_MIN_LEN ||
      trimmedGenre.length > Constants.MOVIE_GENRE_MAX_LEN
    ) {
      return res
        .status(400)
        .json({ message: Messages.MOVIE_ERR_GENRE_LEN, movies: [] });
    }

    const trimmedCast = cast.trim();
    if (
      trimmedCast.length < Constants.MOVIE_CAST_MIN_LEN ||
      trimmedCast.length > Constants.MOVIE_CAST_MAX_LEN
    ) {
      return res
        .status(400)
...
router.get("/all", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (movies.length === 0) {
      return res
        .status(404)
        .json({ message: Messages.MOVIE_ERR_NOT_FOUND_ALL, movies: [] });
    }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (isNaN(movieId) || movieId < Constants.TYPICAL_MIN_ID) {
        return res
          .status(400)
          .json({ message: Messages.MOVIE_ERR_ID, movies: [] });
      }
      ...
      if (!movie) {
        return res
          .status(404)
          .json({ message: Messages.MOVIE_ERR_NOT_FOUND, movies: [] });
      }
      res.send({ movies: [movie] });
    } catch (error: any) {
      next(error);
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (viewingFormat !== undefined) {
        if (typeof viewingFormat !== "string")
          return res
            .status(400)
            .json({ message: Messages.MOVIE_ERR_TYPING, movies: [] });
        if (!isViewingFormatCorrect(viewingFormat)) {
      ...
      if (language !== undefined) {
        ...
        if (
          trimmed.length < Constants.MOVIE_LANG_MIN_LEN ||
          trimmed.length > Constants.MOVIE_LANG_MAX_LEN
        ) {
          return res
            .status(400)
            .json({ message: Messages.MOVIE_ERR_LANG_LEN, movies: [] });
      ...
      if (genre !== undefined) {
        ...
        if (
          trimmed.length < Constants.MOVIE_GENRE_MIN_LEN ||
          trimmed.length > Constants.MOVIE_GENRE_MAX_LEN
        ) {
          return res
            .status(400)
            .json({ message: Messages.MOVIE_ERR_GENRE_LEN, movies: [] });
      ...
      if (cast !== undefined) {
        ...
        if (
          trimmed.length < Constants.MOVIE_CAST_MIN_LEN ||
          trimmed.length > Constants.MOVIE_CAST_MAX_LEN
        ) {
          return res
            .status(400)
            .json({ message: Messages.MOVIE_ERR_CAST_LEN, movies: [] });
